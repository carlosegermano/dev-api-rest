...
public class SwaggerConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).useDefaultResponseMessages(false)
				...
				.select().apis(RequestHandlerSelectors.basePackage("com.cursorestapi.apirest.resources"))
				.paths(PathSelectors.any()).build().apiInfo(metaData());
	}

	private ApiInfo metaData() {
		return new ApiInfoBuilder().title("Documentação Curso Dev-Api-Rest")
				.description("API utilizada no curso Dev-Api-Rest: CRdb - Classificações e Reviews de Disciplinas")
				.version("Versão 1.0")
				.contact(new Contact("Carlos Eduardo", "https://github.com/carlosegermano/dev-api-rest",
